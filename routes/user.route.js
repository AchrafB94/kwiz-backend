...
const transporter = nodemailer.createTransport({
  ...
  port: 587,
  auth: {
    user: "raymundo98@ethereal.email",
    pass: "Ecxf5uqFDJaSQZB388",
  },
});
...
users.post("/register", (req, res) => {
  const userData = {
    ...
    status: "verified", //should be "unverified", deactivated for demo
    roleId: 1,
  };
...
users.post("/login", (req, res) => {
  User.findOne({
    ...
    include: {
      model: Role,
      attributes: ["id", "name"],
      include: [{ model: Permission, attributes: ["id", "ruleId", "roleId"] }],
    },
  })
    .then((user) => {
      if (user) {
...
users.get("/", verifyToken, (req, res) => {
  User.findAll({ include: [{ model: School }, { model: Level }] }).then(
    (user) => res.json(user)
  );
});
...
users.get("/", verifyToken, (req, res) => {
  User.findAll({ order: Sequelize.literal("id ASC") }).then((user) =>
    res.json(user)
  );
});
